...
int main ()
{
	...
	ZeroInHullOptimized zeroInHullOptimized(points);
	Solution solutionOptimized = zeroInHullOptimized.check();
	c = Point2D(0,0);
	cout << "Execution time : " << (((float)solutionOptimized.time)/CLOCKS_PER_SEC) << "s" << endl;
	if (solutionOptimized.status == FEASIBLE)
	{	
		Coeffs u = *(solutionOptimized.u);
		cout << "0 is in the convexhull and u = (";
		for (int index = 0; index < points.size(); index++)
		{
			bool zero = false;
